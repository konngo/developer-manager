...
$.dore = function (element, options) {
  ...
  function init() {
    ...
    /* 03.32. Html Editors */
    if (typeof Quill !== "undefined" && $("#quillEditor").length) {
      ...
      // 关闭 bubble 主题 防止报错 提示 quillEditorBubble不存在
    ...
    if (typeof ClassicEditor !== "undefined" && $("#ckEditorClassic").length) {
      console.log("ckEditorClassic")
      // 图片适配器
      class UploadAdapter {
        constructor(loader) {
          this.loader = loader;
      ...
      ClassicEditor.create(document.querySelector("#ckEditorClassic"),{
        ckfinder:{
          uploadUrl:host+"ckeditor-upload"
        }
      }).then(editor => {
        ckEditor = editor;
        // 这个地方加载了适配器
